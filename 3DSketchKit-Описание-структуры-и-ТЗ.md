...
```
Assets/3DSketchKit/
├── README.md                      # Краткий quick start (англ.)
...
| Файл                         | Назначение                                                                                                                   |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `AbilityManager.cs`          | Хранит слоты типов способностей, создаёт экземпляры через `Activator`, вызывает `Tick`/`Perform*`, реализует `IAbilityHost`. |
...
| Файл                    | Назначение                                                                                                               |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `WalkAbility.cs`        | `ITickableAbility`: читает `MovementInput` с хоста, движение относительно `Camera.main` (если есть), пишет в `IMovable`. |
| `JumpAbility.cs`        | Импульс прыжка: `Rigidbody.linearVelocity` по Y или запасной вариант через `IMovable`.                                   |
| `MeleeAttackAbility.cs` | `OverlapSphere`: урон по `HealthComponent` в радиусе, исключая владельца.                                                |
...
| Файл | Назначение |
|------|------------|
| `AbilityTypeDiscovery.cs` | Поиск по домену приложения классов с `IAbility` и параметрическим конструктором `()`. |
...
| Файл | Назначение |
|------|------------|
| `Windows/SketchKitEditorWindow.cs` | Окно: комната из выделения, добавление слота способности, кнопки создания SO. |
...
- Расширение пользователем: слоты способностей используют `Type.GetType(assemblyQualifiedName)` — типы из **других** сборок должны быть доступны домену (обычно через asmdef-ссылку или плагин в Player).
...
| Расширяемость без правки ассета (свои типы + интерфейсы + слоты) | **частично** (нет отдельного рантайм-плагин-сканера; используются qualified имена типов) |
